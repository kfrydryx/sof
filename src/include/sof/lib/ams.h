...
#define AMS_MESSAGE_UUID_SIZE 16
...
struct uuid_idx {
	uint32_t message_type_id;
	uint8_t message_uuid[AMS_MESSAGE_UUID_SIZE];
};

struct ams_shared_context {
	/* shmid should be only used with ams_acquire/relase function, not generic ones */
	struct coherent c;
	...
	struct ams_slot __aligned(0x1000) slots[CONFIG_CORE_COUNT];
};

struct ams_context {
	/* shared context must be always accessed with shared->shmid taken */
	struct ams_shared_context __sparse_cache *shared;
	/* ams_send_ixc_fn send_message_callback; */
	void *callback_context;
	uint32_t pending_slots_;
};

struct async_pin_props {
	uint8_t message_type_uuid[16];
};
...
static inline int ams_get_message_type_id(const uint8_t message_uuid[AMS_MESSAGE_UUID_SIZE],
...
static inline int send_message_over_ixc(struct async_message_service *ams, uint32_t slot,
